...
- **Modal Design:** The document reading experience must feel like a dedicated "focus mode" without breaking the metaphor that the user is still inside the Command Center. For long documents, this includes structural wayfinding (ToC).
...
**2. The Markdown Document Modal (The "Project Viewer")**
...
- **Reading Rhythm & Spacing:**
  - **Generous Margins**: Paragraphs use `mb-10` to prevent "wall of text" fatigue.
  - **Sector Breaks**: H2/H3 headers use `mt-16` to provide clear visual separation between sections.
  - **Line Density**: `leading-loose` ensures technical prose remains legible.
- **Wayfinding (The Side-Rail):**
  - **Interactive ToC**: A persistent (desktop) sidebar extracts headers for immediate structural jumping.
...
- **Adaptive Interactions (Story 7.7):**
  - **Persistent Header:** The document header maintains a stable size during scroll to provide a predictable reading environment and avoid layout shift bugs.
  - **Sidebar Auto-Scrolling:** To maintain "Active Item Centering," the TOC sidebar automatically scrolls internally to keep the currently highlighted heading visible and centered within the sidebar container.
  - **Stickiness Alignment:** The TOC sidebar "Contents" title is explicitly offset (via measured `headerHeight`) to stick just below the bottom edge of the persistent header, preventing any visual overlap.
...
- **Close behavior:** The modal can be closed via the Back button, Escape key, clicking the overlay backdrop, or pressing the browser back button. All close methods remove only the `?document=` parameter from the URL — all other filter state (`?project=`, `?domain=`, `?tech=`) is preserved.

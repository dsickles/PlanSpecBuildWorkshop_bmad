...
Content`;
        ...
        const article = result as ParsedArticle;
        expect(article.links).toBeDefined();
        expect(article.links).toHaveLength(2);
        expect(article.links?.[0].label).toBe("GitHub");
        expect(article.links?.[0].url).toBe("https://github.com/lovable-dev/lovable");
    });
...
Content`;
        ...
        if ('_error' in result) {
            expect(result._message).toContain("Link must be a valid URL");
        }
    });
});
